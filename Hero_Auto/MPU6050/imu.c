...
void Init_Quaternion()//根据测量数据，初始化q0,q1,q2.q3，从而加快收敛速度
{
	...
	*/
	...
	// X -155deg Why :) whatever we solved it
	/*
	int16_t hx,hy,hz;
...
void IMU_AHRSupdate(void) {
    ...
    ez = -(ax*vy - ay*vx); //cross(v,a)
		...
		errAng = -0.001*atan2((1.0f/norm),(vx*ax+vy*ay+vz*az)); //errAng = atan(cross(A,B)/dot(A,B)),to eliminate the obtuse angle problem
		qe0 = cos(errAng/2.0f);
...
{  
    ...
    angles[0] = -atan2(2 * q[1] * q[2] + 2 * q[0] * q[3], -2 * q[2]*q[2] - 2 * q[3] * q[3] + 1)* 180/M_PI; // yaw        -pi----pi note the minus sign??? 
