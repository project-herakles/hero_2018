...
void MouseKeyControlProcess(Mouse *mouse, Key *key)
{
    ...
    if(GetWorkState() == NORMAL_STATE)
    {
        ...
        GimbalRef.pitch_angle_dynamic_ref -= (float)mouse->y* MOUSE_TO_PITCH_ANGLE_INC_FACT;  //(rc->ch3 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_PITCH_ANGLE_INC_FACT;
        GimbalRef.yaw_angle_dynamic_ref   += (float)mouse->x* MOUSE_TO_YAW_ANGLE_INC_FACT;
...
void Control_Mode_Switch(RC_Ctl_t* data)
{
	...
	if(data->key.v&0x15)//SWITCH ACTION
	{
		if(duckMode == NORMALPOSITION)
			duckMode = DUCKING;
		else if(duckMode == DUCKING)
			duckMode = NORMALPOSITION;
	}
...
void Move_Control(RC_Ctl_t* data)
{
  ...
  GimbalRef.yaw_speed_ref   = data->mouse.x;
	...
	switch(dir)
	{
		...
		case FULL_LEFTWARD:
			ChassisSpeedRef.forward_back_ref = 0;
			ChassisSpeedRef.left_right_ref = speedRef*0.8;
			break;
		case FULL_RIGHTWARD:
			ChassisSpeedRef.forward_back_ref = 0;
			ChassisSpeedRef.left_right_ref = -speedRef*0.8;
