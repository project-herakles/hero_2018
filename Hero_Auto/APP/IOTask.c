...
void AppParamInit(void)
{
    ...
    //In gAppParamStruct, PID and offsets data are packed
		...
		(PARAM_CALI_DONE == tmp_param.GyroCaliData.GyroCaliFlag))
	{
        ...
        memcpy(&gAppParamStruct, &tmp_param, sizeof(AppParam_t));//If calied, copy it to AppParamStruct for initialization
    }
    else
    {
    ...
    //if not calied before the flag is NONE then init the para with default value
