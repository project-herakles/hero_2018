...
void TIM8_Configuration(void)
{
	...
	// 使能TIM2重载寄存器ARR
  	TIM_ARRPreloadConfig(TIM8, ENABLE);	
	  TIM_PrescalerConfig(TIM8, 0, TIM_PSCReloadMode_Update);//What does this line mean? Set Prescaler to 0 will induce an error in run-time
...
void TIM6_Configuration(void)
{
    ...
    tim.TIM_Prescaler = 84-1;        //84M internal clock, get 1M frequency
    tim.TIM_CounterMode = TIM_CounterMode_Up;
    tim.TIM_ClockDivision = TIM_CKD_DIV1;
    tim.TIM_Period = 1000;  //1ms,1000Hz (checked)
...
void TIM2_Configuration(void)
{
    ...
    tim.TIM_Period = 0xFFFFFFFF; //What is this 0xFFFFFFFF for?
