...
/*-----USART3_TX-----PB10-----*/
/*-----USART3_RX-----PB11-----*/
Receive_Info_e judgement_start_receive_flag = UNSUCCESSFUL;
uint8_t test_step=0;
...
{  
    ...
    {   
        tmp = USART_ReceiveData(USART3);
			  if(judgement_start_receive_flag == SUCCESSFUL)
					judgement_handler(tmp,DN_REG_ID);
				else if(judgement_start_receive_flag == UNSUCCESSFUL)//still receive calibration data
				{
					if(FrameUnpack(tmp, rx_buffer))
					{
							...
							else//start getting judgement info
							{
								 judgement_start_receive_flag = SUCCESSFUL;
								 judgement_handler(tmp,DN_REG_ID);
							}
					}
				else//start getting judgement info (? what is the use? Should never enter)
        {
					     test_step=2;
               judgement_start_receive_flag = SUCCESSFUL;
							 judgement_handler(tmp,DN_REG_ID);
