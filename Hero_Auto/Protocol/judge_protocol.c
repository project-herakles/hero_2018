...
/* Originally protocol.c
 * define CRC16
 * Constants are somewhere defined. 
 * communication protocol of main control with judge system */
...
extern const uint8_t CRC8_INIT;    // = 0xff;
...
extern uint16_t       CRC_INIT;       // = 0xffff;
...
void append_crc8_check_sum(uint8_t* pchMessage, uint16_t dwLength)
{
  ...
  if ((pchMessage == 0) || (dwLength <= 2))
    return;
  ucCRC                    = get_crc8_check_sum((uint8_t*)pchMessage, dwLength - 1, 0xff);
...
void append_crc16_check_sum(uint8_t* pchMessage, uint32_t dwLength)
{
  ...
  wCRC                     = get_crc16_check_sum((uint8_t*)pchMessage, dwLength - 2, 0xffff);
